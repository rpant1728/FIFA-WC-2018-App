...
export default class Match_Detail extends React.Component{    
  ...
  player_profile(name){
    for(var i=0; i<teams.length; i++){
      if(teams[i].country === match.away_team_country){
        curr_team = teams[i];
        break;
      }
    }
  ...
  render(){
    ...
    away_team_lineup = match.away_team_statistics.starting_eleven;
    for(var i=0; i<away_team_lineup.length; i++){
      player_name = away_team_lineup[i].name.split(" ");
      temp = ""
      ...
      away_team_lineup[i].name = temp;
    }
    defenders = away_team_lineup.filter(l =>{
      return l.position.match("Defender");
    });
    forwards = away_team_lineup.filter(l =>{
      return l.position.match("Forward");
    });
    goalie = away_team_lineup.filter(l =>{
      return l.position.match("Goalie");
    });
    midfielders = away_team_lineup.filter(l =>{
